...
namespace cilspirv.Spirv.Ops
{
    ...
    public sealed record OpBitReverse : BitInstruction
    {
        public ID ResultType { get; init; }
        public ID Result { get; init; }
        public ID Base { get; init; }

        public override OpCode OpCode => OpCode.OpBitReverse;
        public override int WordCount => 1 + 1 + 1 + 1 + ExtraWordCount;
        public override ID? ResultID => Result;
        public override ID? ResultTypeID => ResultType;

        public override IEnumerable<ID> AllIDs => new[] { ResultType, Result, Base }.Concat(ExtraIDs);

        public OpBitReverse() {}

        private OpBitReverse(IReadOnlyList<uint> codes, Range range)
        {
