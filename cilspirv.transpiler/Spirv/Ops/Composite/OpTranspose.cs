...
namespace cilspirv.Spirv.Ops
{
    ...
    public sealed record OpTranspose : CompositeInstruction
    {
        public ID ResultType1 { get; init; }
        public ID Result2 { get; init; }
        public ID Matrix { get; init; }

        public override OpCode OpCode => OpCode.OpTranspose;
        public override int WordCount => 1 + 1 + 1 + 1;
        public override ID? ResultID => Result2;
        public override ID? ResultTypeID => ResultType1;

        public override IEnumerable<ID> AllIDs => new[] { ResultType1, Result2, Matrix };

        public OpTranspose() {}

        private OpTranspose(IReadOnlyList<uint> codes, Range range)
        {
