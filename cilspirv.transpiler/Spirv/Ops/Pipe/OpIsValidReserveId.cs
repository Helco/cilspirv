...
namespace cilspirv.Spirv.Ops
{
    ...
    public sealed record OpIsValidReserveId : PipeInstruction
    {
        public ID ResultType { get; init; }
        public ID Result { get; init; }
        public ID ReserveId { get; init; }

        public override OpCode OpCode => OpCode.OpIsValidReserveId;
        public override int WordCount => 1 + 1 + 1 + 1 + ExtraWordCount;
        public override ID? ResultID => Result;
        public override ID? ResultTypeID => ResultType;

        public override IEnumerable<ID> AllIDs => new[] { ResultType, Result, ReserveId }.Concat(ExtraIDs);

        public OpIsValidReserveId() {}

        private OpIsValidReserveId(IReadOnlyList<uint> codes, Range range)
        {
