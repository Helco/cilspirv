...
namespace cilspirv.Spirv.Ops
{
    ...
    public sealed record OpSUDotAccSatKHR : ArithmeticInstruction
    {
        ...
        public override OpCode OpCode => OpCode.OpSUDotAccSatKHR;
        public override int WordCount => 1 + 1 + 1 + 1 + 1 + 1 + (PackedVectorFormat.HasValue ? 1 : 0);
        public override ID? ResultID => Result2;
        public override ID? ResultTypeID => ResultType1;

        public override IEnumerable<ID> AllIDs => new[] { ResultType1, Result2, Vector1, Vector2, Accumulator };

        public OpSUDotAccSatKHR() {}

        private OpSUDotAccSatKHR(IReadOnlyList<uint> codes, Range range)
        {
