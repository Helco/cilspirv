...
{
    public sealed record OpSampledImage : ImageInstruction
    {
        public ID ResultType1 { get; init; }
        public ID Result2 { get; init; }
        public ID Image { get; init; }
        public ID Sampler { get; init; }

        public override OpCode OpCode => OpCode.OpSampledImage;
        public override int WordCount => 1 + 1 + 1 + 1 + 1;
        public override ID? ResultID => Result2;
        public override ID? ResultTypeID => ResultType1;

        public override IEnumerable<ID> AllIDs => new[] { ResultType1, Result2, Image, Sampler };

        public OpSampledImage() {}

        private OpSampledImage(IReadOnlyList<uint> codes, Range range)
        {
