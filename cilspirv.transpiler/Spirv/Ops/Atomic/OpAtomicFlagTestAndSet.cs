...
namespace cilspirv.Spirv.Ops
{
    ...
    public sealed record OpAtomicFlagTestAndSet : AtomicInstruction
    {
        ...
        public override OpCode OpCode => OpCode.OpAtomicFlagTestAndSet;
        public override int WordCount => 1 + 1 + 1 + 1 + 1 + 1 + ExtraWordCount;
        public override ID? ResultID => Result;
        public override ID? ResultTypeID => ResultType;

        public override IEnumerable<ID> AllIDs => new[] { ResultType, Result, Pointer, Memory, Semantics }.Concat(ExtraIDs);

        public OpAtomicFlagTestAndSet() {}

        private OpAtomicFlagTestAndSet(IReadOnlyList<uint> codes, Range range)
        {
