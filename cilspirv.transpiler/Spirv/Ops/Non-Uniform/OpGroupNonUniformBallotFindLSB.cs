...
namespace cilspirv.Spirv.Ops
{
    ...
    public sealed record OpGroupNonUniformBallotFindLSB : NonUniformInstruction
    {
        ...
        public override OpCode OpCode => OpCode.OpGroupNonUniformBallotFindLSB;
        public override int WordCount => 1 + 1 + 1 + 1 + 1;
        public override ID? ResultID => Result2;
        public override ID? ResultTypeID => ResultType1;

        public override IEnumerable<ID> AllIDs => new[] { ResultType1, Result2, Execution, Value };

        public OpGroupNonUniformBallotFindLSB() {}

        private OpGroupNonUniformBallotFindLSB(IReadOnlyList<uint> codes, Range range)
        {
