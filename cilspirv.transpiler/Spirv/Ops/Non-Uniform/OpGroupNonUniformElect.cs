...
namespace cilspirv.Spirv.Ops
{
    ...
    public sealed record OpGroupNonUniformElect : NonUniformInstruction
    {
        public ID ResultType { get; init; }
        public ID Result { get; init; }
        public ID Execution { get; init; }

        public override OpCode OpCode => OpCode.OpGroupNonUniformElect;
        public override int WordCount => 1 + 1 + 1 + 1 + ExtraWordCount;
        public override ID? ResultID => Result;
        public override ID? ResultTypeID => ResultType;

        public override IEnumerable<ID> AllIDs => new[] { ResultType, Result, Execution }.Concat(ExtraIDs);

        public OpGroupNonUniformElect() {}

        private OpGroupNonUniformElect(IReadOnlyList<uint> codes, Range range)
        {
