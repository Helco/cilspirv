...
namespace cilspirv.Spirv.Ops
{
    ...
    public sealed record OpGroupNonUniformBallotBitExtract : NonUniformInstruction
    {
        ...
        public override OpCode OpCode => OpCode.OpGroupNonUniformBallotBitExtract;
        public override int WordCount => 1 + 1 + 1 + 1 + 1 + 1 + ExtraWordCount;
        public override ID? ResultID => Result;
        public override ID? ResultTypeID => ResultType;

        public override IEnumerable<ID> AllIDs => new[] { ResultType, Result, Execution, Value, Index }.Concat(ExtraIDs);

        public OpGroupNonUniformBallotBitExtract() {}

        private OpGroupNonUniformBallotBitExtract(IReadOnlyList<uint> codes, Range range)
        {
